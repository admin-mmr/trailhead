...
Slug resolution + job-state getters          → sync_worker_helpers.py
This file: the background worker, Steps 2+3 calls, DB persistence, spawn.
"""
...
# Re-imported so the public import surface of `sync_worker` is unchanged and the
# background worker below can call these by bare name. `_jobs`/`_jobs_lock` are
# the SAME objects the status getters mutate — item mutation on the shared dict.
from sync_worker_helpers import (  # noqa: F401  (re-exported public API)
    _jobs,
    _jobs_lock,
    _resolve_slug_to_event,
    _resolve_slug_to_canonical,
    _compute_progress,
    get_job_status,
    get_all_jobs,
    cancel_job,
