...
export function PaymentStep(props: PaymentStepProps) {
  ...
  return (
    <form onSubmit={onSubmit}>
        ...
        {([
          ...
          <button key={m.id} type="button"
            ...
            className={`flex-1 py-2 rounded-xl border-2 font-semibold transition-colors
              ${payMethod === m.id ? 'border-brand-crimson bg-lantern-gold-tint text-brand-crimson' : 'border-lantern-line text-lantern-ink-soft hover:border-lantern-gold-foil'}`}>
            {m.label}
          </button>
        ))}
      </div>
      ...
      {payMethod === 'card' && (
        <div className="bg-lantern-blush rounded-xl p-6 mb-6 text-center">
          <p className="text-2xl font-bold text-brand-crimson mb-2">${donateAmount}</p>
          <p className="text-sm text-lantern-ink-soft">
            {lang === 'zh'
      ...
      <>
      <div className="bg-lantern-blush rounded-xl p-6 mb-6 text-center">
        <div className="w-40 h-40 bg-white border-2 border-dashed border-lantern-line rounded-xl mx-auto flex items-center justify-center overflow-hidden">
          <Image
        ...
        <p className="text-sm text-lantern-ink-soft mt-2">
          {lang === 'zh' ? '扫描二维码付款' : 'Scan QR code to pay'}
        </p>
        <div className="mt-4 space-y-1">
          <p className="font-semibold text-lantern-ink text-lg">
            {payMethod === 'zelle' ? zelleHandle : venmoHandle}
          </p>
          <p className="text-2xl font-bold text-brand-crimson">${donateAmount}</p>
      ...
      <div className="flex gap-4 mt-8">
        ...
        <button type="submit" disabled={submitting}
          className="flex-1 bg-brand-crimson text-white py-3 rounded-xl font-semibold hover:bg-brand-crimson-dark transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
