...
export function DoneStep({ lang, memberId, eventId }: DoneStepProps) {
  return (
    <div className="text-center py-6">
      ...
      <h2 className="text-2xl font-bold text-lantern-ink mb-2">
        {lang === 'zh' ? '申请已提交！' : 'Application Submitted!'}
      </h2>
      <p className="text-lantern-ink-soft mb-4">
        {lang === 'zh'
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-6">
        {memberId && (
          <div className="inline-block bg-green-50 border border-green-200 rounded-xl px-6 py-3">
            <p className="text-xs text-lantern-ink-soft uppercase tracking-wider">
        ...
        {eventId && (
          <div className="inline-block bg-lantern-blush border border-lantern-line rounded-xl px-6 py-3">
            <p className="text-xs text-lantern-ink-soft uppercase tracking-wider">
              {lang === 'zh' ? '参考号' : 'Reference Number'}
            </p>
            <p className="text-lg font-mono font-bold text-lantern-ink mt-1">{eventId}</p>
          </div>
        )}
      </div>
      ...
      <a href="/login"
        className="mt-6 inline-block bg-brand-crimson text-white px-8 py-3 rounded-xl font-semibold hover:bg-brand-crimson-dark transition-colors">
