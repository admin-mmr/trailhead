...
function rowToMemberObject(row: any[]): any {
  return {
    ...
    Created: row[MM_COL.CREATED],
    Expiration: row[MM_COL.EXPIRATION],
...
function rowToEventObject(row: any[]): any {
  return {
    ...
    Timestamp: row[WE_COL.TIMESTAMP],
    ExpiresAt: row[WE_COL.EXPIRES_AT],
    ...
    ApprovalDate: row[WE_COL.APPROVAL_DATE],
    Notes: row[WE_COL.NOTES],
    PaymentDate: row[WE_COL.PAYMENT_DATE],
