...
function initiateSwitch(jsonRequest) {
    ...
    try {
        ...
        if (member.status === 'pending_upgrade') {
            return jsonError(req.requestId, 'INVALID_STATE', 'A family upgrade is already in progress. Complete or cancel it first.');
        }
        if (member.type === 'Family' && member.status === 'active') {
            return jsonError(req.requestId, 'INVALID_STATE', 'Already an active Family member.');
        }
        // Log before write
...
function initiateUpgrade(jsonRequest) {
    const req = JSON.parse(jsonRequest);
    const { payload } = req;
    try {
        ...
        if (member.status !== 'active') {
            return jsonError(req.requestId, 'INVALID_STATE', 'Family Upgrade requires an active membership. Use Switch to Family instead.');
        }
        if (member.type !== 'Individual') {
            return jsonError(req.requestId, 'INVALID_STATE', 'Already on a Family plan.');
        }
        ...
        if (isNaN(expDate.getTime()) || expDate <= minExpiration) {
            return jsonError(req.requestId, 'INVALID_STATE', `Upgrade to Family requires more than ${upgradeMinMonths} months remaining on your membership. ` +
...
function cancelUpgrade(jsonRequest) {
    const req = JSON.parse(jsonRequest);
    const { payload } = req;
    try {
        ...
        const { member } = result;
        if (member.status !== 'pending_upgrade') {
            return jsonError(req.requestId, 'INVALID_STATE', 'No pending upgrade to cancel.');
        }
        ...
        for (const ev of pendingEvents) {
            if (ev.memberID === payload.memberID &&
...
globalThis.initiateSwitch = initiateSwitch;
globalThis.initiateUpgrade = initiateUpgrade;
globalThis.cancelUpgrade = cancelUpgrade;
