...
// ============================================================
function submitDuesPayment(jsonRequest) {
    var _a, _b;
    const req = JSON.parse(jsonRequest);
    const { payload } = req;
    try {
        console.log('[mmr][submitDuesPayment] memberID:', payload.memberId, '| intent:', payload.paymentIntent, '| amount:', payload.amount);
        ...
        const eventID = appendWebAppEvent({
            ...
            last4Digits: (_a = payload.last4Digits) !== null && _a !== void 0 ? _a : '',
            familyMemberEmails: (_b = payload.familyMemberEmails) !== null && _b !== void 0 ? _b : '',
            status: 'Pending',
            matchedMessageId: '',
...
function submitRenewalRequest(jsonRequest) {
    return submitDuesPayment(jsonRequest);
}
function reconcileWebAppWithGmail(_jsonRequest) {
    try {
...
function findGmailMatch(event, gmailRows) {
    const eventDate = new Date(event.timestamp);
    const SEVEN_DAYS_MS = 7 * 24 * 60 * 60 * 1000;
    for (const row of gmailRows) {
        if (row.processed)
            continue;
        if (row.amount !== event.amount)
            continue;
        const rowDate = new Date(row.transactionDate || row.timestamp);
        if (isNaN(rowDate.getTime()))
            continue;
        if (Math.abs(eventDate.getTime() - rowDate.getTime()) > SEVEN_DAYS_MS)
            continue;
        const trimmed4 = (event.last4Digits || '').trim();
        const last4Match = trimmed4.length === 4 && (row.transactionNumber || '').endsWith(trimmed4);
        ...
        const payerNameMatch = payerLower.length > 0 && senderLower.length > 0 &&
...
function approveDuesPayment(jsonRequest) {
    var _a, _b;
    const req = JSON.parse(jsonRequest);
    ...
    try {
        ...
        if (intent === 'Family Upgrade') {
            ...
            if (primary.member.status !== 'pending_upgrade') {
                return jsonError(req.requestId, 'INVALID_STATE', 'Family Upgrade approval requires the member to be in pending_upgrade state.');
            }
            ...
            updateWebAppEventRow(found.rowIndex, {
                ...
                APPROVAL_DATE: now,
                NOTES: (_a = payload.notes) !== null && _a !== void 0 ? _a : '',
            });
        ...
        for (const { member } of membersToUpdate) {
            if (member.expiration) {
                ...
                if (!isNaN(current.getTime()) && current > today) {
                    ...
                    if (extended > newExpiration)
                        newExpiration = extended;
                }
            }
        }
        ...
        updateWebAppEventRow(found.rowIndex, {
            ...
            ADMIN_APPROVER: payload.adminEmail,
            APPROVAL_DATE: now,
            NOTES: (_b = payload.notes) !== null && _b !== void 0 ? _b : '',
        });
...
// Backward-compat alias
function approveRenewal(jsonRequest) {
    return approveDuesPayment(jsonRequest);
}
function baseRecord(event, payload) {
    var _a;
    return {
...
function rejectDuesPayment(jsonRequest) {
    const req = JSON.parse(jsonRequest);
    const { payload } = req;
    try {
...
// Backward-compat alias
function rejectRenewal(jsonRequest) {
    return rejectDuesPayment(jsonRequest);
}
globalThis.submitDuesPayment = submitDuesPayment;
globalThis.submitRenewalRequest = submitRenewalRequest;
globalThis.reconcileWebAppWithGmail = reconcileWebAppWithGmail;
globalThis.approveDuesPayment = approveDuesPayment;
globalThis.approveRenewal = approveRenewal;
globalThis.rejectDuesPayment = rejectDuesPayment;
globalThis.rejectRenewal = rejectRenewal;
