...
function getOrCreateMemberProfile(jsonRequest) {
    ...
    try {
        ...
        console.log('[mmr] getOrCreateMemberProfile payload email:', payload.email, 'resolved:', resolvedEmail);
        if (!resolvedEmail) {
...
function updateMemberProfile(jsonRequest) {
    const req = JSON.parse(jsonRequest);
    const { payload } = req;
    try {
        ...
        if (payload.phoneNumber !== undefined)
            updates['PHONE_NUMBER'] = payload.phoneNumber.trim();
        if (payload.wechatID !== undefined)
            updates['WECHAT_ID'] = payload.wechatID.trim();
        if (payload.district !== undefined)
            updates['DISTRICT'] = payload.district.trim();
        if (payload.joinYear !== undefined)
            updates['JOIN_YEAR'] = payload.joinYear.trim();
        ...
        return jsonOk(req.requestId, { member: updated === null || updated === void 0 ? void 0 : updated.member });
    }
    catch (e) {
...
function createNewMember(jsonRequest) {
    const req = JSON.parse(jsonRequest);
    const { payload } = req;
    try {
        ...
        if (!created)
            throw new Error('Failed to retrieve newly created member record.');
...
function getMemberPaymentHistory(jsonRequest) {
    const req = JSON.parse(jsonRequest);
    const payload = req.payload;
    try {
        const email = payload.email.trim().toLowerCase();
...
globalThis.getOrCreateMemberProfile = getOrCreateMemberProfile;
globalThis.updateMemberProfile = updateMemberProfile;
globalThis.getMemberPaymentHistory = getMemberPaymentHistory;
globalThis.createNewMember = createNewMember; // ← 
