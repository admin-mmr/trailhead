...
// ============================================================
function getPendingEvents(jsonRequest) {
...
function getUnmatchedPayments(jsonRequest) {
    const req = JSON.parse(jsonRequest);
    try {
        if (!isAdmin(req.payload.adminEmail)) {
            return jsonError(req.requestId, 'FORBIDDEN', 'Not authorized.');
...
function getPaymentProofs(jsonRequest) {
    const req = JSON.parse(jsonRequest);
    try {
        if (!isAdmin(req.payload.adminEmail)) {
            return jsonError(req.requestId, 'FORBIDDEN', 'Not authorized.');
...
function getConfig(jsonRequest) {
    const req = JSON.parse(jsonRequest);
    try {
...
function updateConfigEntry(jsonRequest) {
    const req = JSON.parse(jsonRequest);
    const { payload } = req;
    try {
...
function getPublicConfig(jsonRequest) {
    var _a;
    const req = JSON.parse(jsonRequest);
    try {
        ...
        for (const key of publicKeys) {
            console.log(`mmr:getPublicConfig key="${key}" value="${(_a = allConfig[key]) !== null && _a !== void 0 ? _a : '(missing)'}" `);
            if (allConfig[key])
...
function manualMatch(jsonRequest) {
    const req = JSON.parse(jsonRequest);
    ...
    try {
        ...
        const updatedEvent = {
            ...
            eventType: String(eventRow[WE_COL.EVENT_TYPE]),
            ...
            paymentIntent: String(eventRow[WE_COL.PAYMENT_INTENT]),
            amount: Number(eventRow[WE_COL.AMOUNT]) || 0,
            paymentMethod: String(eventRow[WE_COL.PAYMENT_METHOD]),
            ...
            status: String(eventRow[WE_COL.STATUS]),
...
globalThis.getPendingEvents = getPendingEvents;
globalThis.getUnmatchedPayments = getUnmatchedPayments;
globalThis.getConfig = getConfig;
globalThis.updateConfigEntry = updateConfigEntry;
globalThis.getPaymentProofs = getPaymentProofs;
globalThis.getPublicConfig = getPublicConfig;
globalThis.manualMatch = manualMatch;
