...
// ============================================================
function handleGoogleLogin(jsonRequest) {
...
function lookupEmail(jsonRequest) {
    const req = JSON.parse(jsonRequest);
    const { payload } = req;
    try {
...
function requestEmailOtp(jsonRequest) {
    const req = JSON.parse(jsonRequest);
    const { payload } = req;
    try {
        const email = payload.email.trim().toLowerCase();
...
function sendCode(email, otpCode) {
    const otpValidHours = parseInt(getConfigValue('OTPValidHours'), 10) || 24;
    MailApp.sendEmail({
        to: email,
...
function handleOtpNotFound(jsonRequest) {
    const req = JSON.parse(jsonRequest);
    const { payload } = req;
    try {
        const email = payload.email.trim().toLowerCase();
...
function verifyEmailOtp(jsonRequest) {
    const req = JSON.parse(jsonRequest);
    const { payload } = req;
    try {
        const email = payload.email.trim().toLowerCase();
...
globalThis.lookupEmail = lookupEmail;
globalThis.requestEmailOtp = requestEmailOtp;
globalThis.verifyEmailOtp = verifyEmailOtp;
