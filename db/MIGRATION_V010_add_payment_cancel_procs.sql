-- =============================================================================
-- MIGRATION V010 — Add sp_cancel_payment and sp_clear_transaction
-- =============================================================================
-- Description:
--   Adds two stored procedures for safe payment reversal:
--     sp_cancel_payment(p_payment_id, p_cancelled_by)
--       — Cancels a single payment; reverts member (+ family) from member_log,
--         reverts submission to pending, clears gmail_transactions link.
--     sp_clear_transaction(p_tx_number, p_dry_run, p_cleared_by)
--       — Clears all payments for a TransactionNumber; same revert logic.
--         p_dry_run=1 returns preview result sets with no writes.
--
-- Safe to re-run: both procs use DROP IF EXISTS before CREATE.
...
-- -----------------------------------------------------------------------------
-- 3. Schema migration self-registration (required)
-- -----------------------------------------------------------------------------

INSERT INTO schema_migrations (version, description, executed_at)
VALUES ('V010', 'Add sp_cancel_payment and sp_clear_transaction stored procedures', NOW())
