...
### 07-30 UTC — HANDOFF: portal login "spinning forever" — NOT root-caused; poll shipped (V039+V040)
Changed/shipped: PR #33 (poll service, V039 **applied+verified**) and PR #34 (full-design links, **V040 NOT YET APPLIED**) are both merged. Poll is live at `/poll/website-design-2026` — verified 11 options render, results 403-gated before voting, bad member ID → 404 generic message. ⚠️ **V040 still needs applying**: `gh workflow run run-db-migrations.yml --ref main -f migration_version=V040`; until then `detail_path` is NULL and the "See the full design" links simply don't render (safe degradation, not a break).
Status — login incident, UNRESOLVED. Reported symptom: `/login?from=%2Fportal` "spins forever". **Could not reproduce; every server path measured healthy.** RULED OUT with evidence: NEXTAUTH_URL is correct in SWA settings (redirects use the real host, not localhost); missing `AUTH_SECRET` is harmless (next-auth 5.0.0-beta.30 `lib/env.js` reads `AUTH_SECRET ?? NEXTAUTH_SECRET`, and NEXTAUTH_SECRET is set); Google OAuth initiation is fine (**POST** `/api/auth/signin/google` → correct 302 to accounts.google.com with right redirect_uri — an earlier GET returning `error=Configuration` was my own red herring, v5 sign-in is POST-only); credentials POST correctly returns `CredentialsSignin`; edge JWT verification works (synthetic active token → `/portal` 200 in 0.63s); expired/pending/inactive → exactly ONE redirect to `/membership/inactive`, no loop; `/membership/inactive` clears its spinner (`setInitialCheck(false)`, page.tsx:71) and `POST /api/auth/refresh-session` returns 200 `{"status":"inactive"}` in 0.44s; `updateMemberOAuthSub` UPDATE 233ms; `createNewMember` path healthy (generate_member_id → A0667, INSERT ok). **LEADING HYPOTHESIS:** the tester's account simply never reaches the portal. `admin@mmrunners.org` = **A0444, Status='inactive', Expiration 2025-02-01** → a successful Google login correctly lands on `/membership/inactive`, which *looks* like "login didn't work". Corroborated: two `member_log` UPDATE rows on A0444 at 19:56 UTC = two real login attempts reaching `/auth/complete` and writing successfully. Separately `it@youth4am.org` is **not a member at all** → signing in with it creates a pending member and redirects to `/join`.
Next: get three facts from the reporter — (1) which Google account, (2) the URL the browser sits on while spinning, (3) browser console/network errors. Then either activate A0444 (or test with an active member) and retry. **Do NOT re-run the createNewMember diagnostic in a transaction:** `CALL generate_member_id()` causes an IMPLICIT COMMIT, so the ROLLBACK silently did nothing and left a real A0667 row + 4 `member_log` rows in prod — both cleaned up (members back to 657, max A0666, next id A0667, no payments/submissions created), but that is a live landmine. Also found, unrelated and unfixed: **`STRIPE_WEBHOOK_SECRET` in SWA settings is 7 chars** (a real one is ≈38+) — the placeholder-paste failure CLAUDE.md already warns about twice, so Stripe webhook signature verification is broken in prod RIGHT NOW and card payments would be banked by Stripe but never recorded; **`JOB_SECRET` is missing** from SWA settings, so PR #32's renewal reminders fail closed and send nothing; **App Insights is configured but receiving zero telemetry**, which is why there were no server logs to read during this investigation.
