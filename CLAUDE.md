...
*Session 2 — calendar (blocked on V037 being applied).* Replace the `/portal/events` placeholder. `GET /api/events/calendar?from=&to=` (`requireActiveMember`, clamp the range server-side to ≤13 months, **default −1 month → +3 months** per the Session 1 finding) → per event: id, event_code, event_name, event_date, location, distance/distance_km, is_virtual, event_url, **my intent**, and running/volunteering counts. Note `distance` is NULL for most upcoming rows (only the marathon has it) — fall back to `distance_km`, then hide the field; don't render "null". Month grid on desktop, chronological list on mobile. EN/中文 via `lib/i18n/translations.ts`. ⚠️ `event_date` is date-only — every render MUST go through `formatLongDate()`/`parseLocalDate()` in `lib/date.ts`; `new Date('2026-11-01')` is UTC midnight and shows a day early west of Greenwich (this already bit the fulfillment emails on 07-29). ⚠️ **`lib/access.ts` has no rule matching `/api/events`** (the existing `/events` public rule does not prefix-match `/api/events`), so it would silently default to `public` — add `{ prefix: '/api/events', tier: 'active' }` in the same commit.

*Session 3 — RSVP + roster.* `POST /api/events/[id]/rsvp` `{intent, note}` → `INSERT … ON DUPLICATE KEY UPDATE` (idempotent by design); `DELETE` clears the row. `GET /api/events/[id]/roster` → names grouped by intent, **honoring `ShowRsvpPublicly`: opted-out members are excluded from the name lists but still counted**. Opt-out toggle added to `/portal/profile`. Admin-side volunteer-coverage view is explicitly out of scope this round (the data is queryable via the Data Browser).
