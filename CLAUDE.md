...
## COMMUNITY POLL SERVICE (webapp) — built 07-30, ⚠️ MIGRATION NOT YET APPLIED
...
**Schema (`MIGRATION_V038`):** `polls` (mode `single|top3`, status `draft|open|closed`, `results_visibility` `after_vote|public|admin`, `voter_check` `member|open`) → `poll_options` → `poll_ballots` → `poll_ballot_choices`. ⚠️ **V038 is written but NOT applied** — run `gh workflow run run-db-migrations.yml --ref main -f migration_version=V038`, then verify against the DB and regenerate `db/schema_snapshot.sql`. Until it runs, `/poll/*` returns a clean 500.
