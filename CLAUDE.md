...
**P1m — NYRR match hygiene — ✅ DONE 07-30 (first pass; a review backlog remains)**
1. **Tier-2 year fix, shipped.** `api_events_match.py` compared `YEAR(CURDATE()) - YearBorn` against `er.age`, but `age` is age **at race time**. The UPDATE is scoped to one event, so `event_year` is a constant — it's now fetched as `COALESCE(event_year, YEAR(event_date))` and passed as a parameter (no extra join). A NULL year makes the age branch false, so a member with a birth year is left for the human queue rather than guessed at. `er.age IS NULL` behaves the same, deliberately. Tests: `mmr-admin/tests/test_events_match_age_year.py` (10) — note they strip `--` comments before asserting, because the comments quote the old buggy expression.
   - ⚠️ **The bug was latent, not the cause of the bad links.** Only **33 members have `YearBorn` and none of them are linked**, so the age branch has effectively never fired; nearly every member reaches Tier 2 via the "no birth year → skip validation" path. Fixing it matters going forward (as `YearBorn` gets populated by the P1L link form) but it explains none of the existing damage.
2. **Audit tool, shipped:** `mmr-admin/audit_nyrr_links.py` — read-only by default, `--unlink` opt-in, `--member/--method/--min-deviation/--min-modal-agreement` filters. Tests: `tests/test_audit_nyrr_links.py` (11).
   - **No stored birth year to check against**, so the reference is each member's **modal implied birth year** (`event_year - age`). Members with <2 rows are never flagged. `--min-modal-agreement` excludes members whose modal cluster is too weak to trust (A0034: 17/54 rows agreed — selectively unlinking "the others" there could delete the *correct* rows).
   - **Real scale was far worse than the 8 members first reported: 311 inconsistent rows across 73 of 360 members.** Failure rate by tier: `auto_lastname` **29%** (60/209), `auto_name` 9%, `auto_firstlast` 7%, even `manual` 7%. 274 of 311 deviated by >5 years, i.e. genuinely different people rather than NYRR's birthday rounding.
   - **Applied policy B on 07-30: 80 rows unlinked** (>5yr deviation AND ≥60% modal agreement) → `mmr_member_id=NULL`, `match_method='unmatched'`, `matched_by/matched_at` cleared. Nothing deleted; **exact revert in `db/archive/REVERT_nyrr_link_audit_20260730.sql`** (generated from live values immediately before the write). Verified: 3731→3651 linked rows, all 80 rows still present and queued, no member lost every link, policy-B re-audit clean, A0077 went from ages 36–65 to a consistent 36–42 across 2020–2026.
   - Also repaired **8 pre-existing stale `nyrr_events.mmr_matched_count`** values (all undercounts, none caused by this unlink — derived data, so no revert needed).
3. **Remaining backlog (not done):** **231 inconsistent rows across 47 members** still linked, including **40 members whose modal cluster is under 60%** — those need a human to review the whole set rather than an automated rule. Run `python3 mmr-admin/audit_nyrr_links.py` for the current list. ⚠️ **Do NOT use `NYRRRunnerName` disagreeing with `FirstName/LastName` as a bad-match signal** — 18 members have a legitimate mismatch, because Chinese members commonly hold an English given name in the club record and their pinyin/legal name at NYRR ('David (Yi) Wan' vs 'Yi Wan', 'Rui (Rae) Qi' vs 'Rae Qi'). Age consistency is the reliable signal.
